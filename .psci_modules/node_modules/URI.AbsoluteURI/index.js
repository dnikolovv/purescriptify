"use strict";
var Control_Apply = require("../Control.Apply/index.js");
var Data_Array = require("../Data.Array/index.js");
var Data_Eq = require("../Data.Eq/index.js");
var Data_Functor = require("../Data.Functor/index.js");
var Data_Generic_Rep = require("../Data.Generic.Rep/index.js");
var Data_Identity = require("../Data.Identity/index.js");
var Data_Lens_Lens = require("../Data.Lens.Lens/index.js");
var Data_Maybe = require("../Data.Maybe/index.js");
var Data_Ord = require("../Data.Ord/index.js");
var Data_Ordering = require("../Data.Ordering/index.js");
var Data_Show_Generic = require("../Data.Show.Generic/index.js");
var Data_String_Common = require("../Data.String.Common/index.js");
var Text_Parsing_Parser = require("../Text.Parsing.Parser/index.js");
var Text_Parsing_Parser_Combinators = require("../Text.Parsing.Parser.Combinators/index.js");
var Text_Parsing_Parser_String = require("../Text.Parsing.Parser.String/index.js");
var URI_Common = require("../URI.Common/index.js");
var URI_HierarchicalPart = require("../URI.HierarchicalPart/index.js");
var URI_Query = require("../URI.Query/index.js");
var URI_Scheme = require("../URI.Scheme/index.js");

// | A strictly absolute URI. An absolute URI can still contain relative paths
// | but is required to have a `Scheme` component.
var AbsoluteURI = (function () {
    function AbsoluteURI(value0, value1, value2) {
        this.value0 = value0;
        this.value1 = value1;
        this.value2 = value2;
    };
    AbsoluteURI.create = function (value0) {
        return function (value1) {
            return function (value2) {
                return new AbsoluteURI(value0, value1, value2);
            };
        };
    };
    return AbsoluteURI;
})();

// | A printer for an absolute URI.
var print = function (opts) {
    return function (v) {
        return Data_String_Common.joinWith("")(Data_Array.catMaybes([ new Data_Maybe.Just(URI_Scheme.print(v.value0)), new Data_Maybe.Just(URI_HierarchicalPart.print(opts)(v.value1)), Data_Functor.map(Data_Maybe.functorMaybe)(function ($105) {
            return URI_Query.print(opts.printQuery($105));
        })(v.value2) ]));
    };
};

// | A parser for an absolute URI.
var parser = function (opts) {
    return Control_Apply.applyFirst(Text_Parsing_Parser.applyParserT(Data_Identity.monadIdentity))(Control_Apply.apply(Text_Parsing_Parser.applyParserT(Data_Identity.monadIdentity))(Control_Apply.apply(Text_Parsing_Parser.applyParserT(Data_Identity.monadIdentity))(Data_Functor.map(Text_Parsing_Parser.functorParserT(Data_Identity.functorIdentity))(AbsoluteURI.create)(URI_Scheme.parser))(URI_HierarchicalPart.parser(opts)))(Text_Parsing_Parser_Combinators.optionMaybe(Data_Identity.monadIdentity)(URI_Common.wrapParser(Data_Identity.monadIdentity)(opts.parseQuery)(URI_Query.parser))))(Text_Parsing_Parser_String.eof(Data_Identity.monadIdentity));
};
var genericAbsoluteURI = {
    to: function (x) {
        return new AbsoluteURI(x.value0, x.value1.value0, x.value1.value1);
    },
    from: function (x) {
        return new Data_Generic_Rep.Product(x.value0, new Data_Generic_Rep.Product(x.value1, x.value2));
    }
};
var showAbsoluteURI = function (dictShow) {
    return function (dictShow1) {
        return function (dictShow2) {
            return function (dictShow3) {
                return function (dictShow4) {
                    return {
                        show: Data_Show_Generic.genericShow(genericAbsoluteURI)(Data_Show_Generic.genericShowConstructor(Data_Show_Generic.genericShowArgsProduct(Data_Show_Generic.genericShowArgsArgument(URI_Scheme.showScheme))(Data_Show_Generic.genericShowArgsProduct(Data_Show_Generic.genericShowArgsArgument(URI_HierarchicalPart.showHierarchicalPart(dictShow)(dictShow1)(dictShow2)(dictShow3)))(Data_Show_Generic.genericShowArgsArgument(Data_Maybe.showMaybe(dictShow4)))))({
                            reflectSymbol: function () {
                                return "AbsoluteURI";
                            }
                        }))
                    };
                };
            };
        };
    };
};
var eqAbsoluteURI = function (dictEq) {
    return function (dictEq1) {
        return function (dictEq2) {
            return function (dictEq3) {
                return function (dictEq4) {
                    return {
                        eq: function (x) {
                            return function (y) {
                                return Data_Eq.eq(URI_Scheme.eqScheme)(x.value0)(y.value0) && Data_Eq.eq(URI_HierarchicalPart.eqHierarchicalPart(dictEq)(dictEq1)(dictEq2)(dictEq3))(x.value1)(y.value1) && Data_Eq.eq(Data_Maybe.eqMaybe(dictEq4))(x.value2)(y.value2);
                            };
                        }
                    };
                };
            };
        };
    };
};
var ordAbsoluteURI = function (dictOrd) {
    return function (dictOrd1) {
        return function (dictOrd2) {
            return function (dictOrd3) {
                return function (dictOrd4) {
                    return {
                        compare: function (x) {
                            return function (y) {
                                var v = Data_Ord.compare(URI_Scheme.ordScheme)(x.value0)(y.value0);
                                if (v instanceof Data_Ordering.LT) {
                                    return Data_Ordering.LT.value;
                                };
                                if (v instanceof Data_Ordering.GT) {
                                    return Data_Ordering.GT.value;
                                };
                                var v1 = Data_Ord.compare(URI_HierarchicalPart.ordHierarchicalPart(dictOrd)(dictOrd1)(dictOrd2)(dictOrd3))(x.value1)(y.value1);
                                if (v1 instanceof Data_Ordering.LT) {
                                    return Data_Ordering.LT.value;
                                };
                                if (v1 instanceof Data_Ordering.GT) {
                                    return Data_Ordering.GT.value;
                                };
                                return Data_Ord.compare(Data_Maybe.ordMaybe(dictOrd4))(x.value2)(y.value2);
                            };
                        },
                        Eq0: function () {
                            return eqAbsoluteURI(dictOrd.Eq0())(dictOrd1.Eq0())(dictOrd2.Eq0())(dictOrd3.Eq0())(dictOrd4.Eq0());
                        }
                    };
                };
            };
        };
    };
};

// | The scheme component of an absolute URI.
var _scheme = function (dictStrong) {
    return Data_Lens_Lens.lens(function (v) {
        return v.value0;
    })(function (v) {
        return function (s) {
            return new AbsoluteURI(s, v.value1, v.value2);
        };
    })(dictStrong);
};

// | The query component of an absolute URI.
var _query = function (dictStrong) {
    return Data_Lens_Lens.lens(function (v) {
        return v.value2;
    })(function (v) {
        return function (q) {
            return new AbsoluteURI(v.value0, v.value1, q);
        };
    })(dictStrong);
};

// | The hierarchical-part component of an absolute URI.
var _hierPart = function (dictStrong) {
    return Data_Lens_Lens.lens(function (v) {
        return v.value1;
    })(function (v) {
        return function (h) {
            return new AbsoluteURI(v.value0, h, v.value2);
        };
    })(dictStrong);
};
module.exports = {
    AbsoluteURI: AbsoluteURI,
    parser: parser,
    print: print,
    "_scheme": _scheme,
    "_hierPart": _hierPart,
    "_query": _query,
    eqAbsoluteURI: eqAbsoluteURI,
    ordAbsoluteURI: ordAbsoluteURI,
    genericAbsoluteURI: genericAbsoluteURI,
    showAbsoluteURI: showAbsoluteURI,
    Authority: URI_HierarchicalPart.Authority,
    HierarchicalPartAuth: URI_HierarchicalPart.HierarchicalPartAuth,
    HierarchicalPartNoAuth: URI_HierarchicalPart.HierarchicalPartNoAuth,
    IPv4Address: URI_HierarchicalPart.IPv4Address,
    IPv6Address: URI_HierarchicalPart.IPv6Address,
    NameAddress: URI_HierarchicalPart.NameAddress,
    Path: URI_HierarchicalPart.Path,
    PathAbsolute: URI_HierarchicalPart.PathAbsolute,
    PathRootless: URI_HierarchicalPart.PathRootless,
    "_IPv4Address": URI_HierarchicalPart["_IPv4Address"],
    "_IPv6Address": URI_HierarchicalPart["_IPv6Address"],
    "_NameAddress": URI_HierarchicalPart["_NameAddress"],
    "_authority": URI_HierarchicalPart["_authority"],
    "_hierPath": URI_HierarchicalPart["_hierPath"],
    "_hosts": URI_HierarchicalPart["_hosts"],
    "_path": URI_HierarchicalPart["_path"],
    "_userInfo": URI_HierarchicalPart["_userInfo"]
};
